function [U, G, Xhat, L1met, stats]=L1HOSVD(X, Ks, Uin, varargin)%U,maxit,tol)
...
%% Calculate initial L1met
G = ttm(X,U,1:n,'t');
% G = tensor(X); % for i=1:n %    G = ttm(G, U{i}', i);  %end
...
G = ttm(X,U,1:n,'t');
% G = tensor(X); % for i=1:n  %    G=ttm(G, U{i}', i);  % end
Xhat = double(ttm(G,U,1:n));
% Xhat=tensor(G);  % for i=1:n  %    Xhat=ttm(Xhat,U{i},i);  % end  % Xhat=double(Xhat);
met = sqrt(sum(abs(G(:)).^2));    % L2 norm of the computed tensor core
